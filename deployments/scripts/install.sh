...
# After version 2.6.3 binaries have name changes, which cause a change in the download link.
# This function checks if it is earlier or later than version 2.6.3 and through this we can decide which is the
...
isOldURLVersion() {
  if [ $VERSION_DOWNLOAD != "latest" ]; then
        ...
        if [ "$VERSION_WITHOUT_DOTS" -gt 263 ]; then
          IS_NEW_URL=true
        fi
  fi
}
